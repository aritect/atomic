...
import { useIsMobile } from "@/hooks-ui-lib/use-mobile";
...
import { MenuCollapseIcon } from "@/icons-ui-lib/menu-collapse-icon";


const SIDEBAR_COOKIE_NAME = "atomic::sidebar";
...
>(
  (
    ...
    const { isMobile, state, openMobile, setOpenMobile, setOpenEnded } = useSidebar();
    const sidebarRef = useCallback((node: HTMLDivElement | null) => {
      if (node) {
        const handleTransitionEnd = (event: TransitionEvent) => {
          if (event.target === node && event.propertyName === "width") {
            setOpenEnded(true);
          }
        };
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className="w-[--sidebar-width] p-4 text-sidebar-foreground [&>button]:hidden"
            style={
              {
...
const SidebarTrigger = forwardRef<
  ElementRef<typeof FunnyButton>,
  ComponentProps<typeof FunnyButton>
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <FunnyButton
      ...
      {...props}
    >
      <MenuCollapseIcon className="size-4" />
