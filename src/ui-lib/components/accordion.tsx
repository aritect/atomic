...
>(({ className, children, chevronBefore, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex w-full">
    ...
    >
      {chevronBefore && (
        <ChevronDownIcon className="h-4 w-4 shrink-0 transition-transform duration-global -rotate-90" />
      )}
      {children}
      {!chevronBefore && (
        <ChevronDownIcon className="h-4 w-4 shrink-0 transition-transform duration-global" />
      )}
    </AccordionPrimitive.Trigger>
